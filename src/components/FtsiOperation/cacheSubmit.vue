<template>
  <div style="width: 100%">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>Cached Task Submit</el-breadcrumb-item>
    </el-breadcrumb>
    <!--  发动机选择栏  -->
    <div class="select_bar">
      ...
      <br>
      <el-button type="primary" size="small" style="margin-top: 15px" @click="openAircraftInfoDialog">Aircraft information change
      </el-button>
    </div>
    ...
    <aircraftInfoDialog ref="aircraftInfoDialogRef"></aircraftInfoDialog>
  </div>
</template>
...
export default {
  name: "cacheSubmit",
  components: {
    aircraftInfoDialog: () => import("./aircraftInfoDialog")
  },
  created() {
    this.getSelectBar()
  },
  ...
  methods: {
    ...
    async checkFTSIinfo(engineForm,validateRef) {
      ...
      validateRef.validate(async valid => {
        if (!valid) return
        //可以发起添加请求
        const {data: res} = await this.$http.get('userOpt/cachedCheckFTSI/', {params: engineForm})
        console.log(res)
    ...
    async submitFTSIinfo(engineForm,validateRef) {
      ...
      validateRef.validate(async valid => {
        if (!valid) return
        //可以发起添加请求
        const {data: res} = await this.$http.put('userOpt/cachedSubmitFTSI/', engineForm)
        if (res.meta.status !== 200) {
    ...
    async openAircraftInfoDialog() {
      const {data: res} = await this.$http.get('userOpt/cacheMSNinfoObtain/',{params:this.queryForm})
      if (res.meta.status !== 200) {
        return this.$message.error(res.meta.msg)
      }
      this.$message.success(res.meta.msg)
      this.$refs.aircraftInfoDialogRef.init(res.data,this.queryForm.aircraftMSN)
    },
  }
}
</script>
